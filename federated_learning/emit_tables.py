...
def _mode_stats(df: pd.DataFrame, mode: str, filters: dict) -> dict:
    sub = df[(df["security_mode"] == mode) & (df["attack"] == "none") & (df["malicious_fraction"] == 0.0)]
...
def emit_byzantine(df: pd.DataFrame, out_path: Path) -> None:
    lines = [
    ...
    for attack, mf, aggs in attack_rows:
        for agg in aggs:
            ...
            if attack == "none" and agg == "fedavg":
                ...
                impact = f"+{pct:.1f}\\% (aggregator CPU cost)"
            else:
                drop = ((acc - base_acc) / base_acc * 100) if base_acc > 0 else 0
                impact = f"Accuracy Drop: $-{abs(drop):.1f}\\%$"
            lines.append(
...
def emit_all_tables(df: pd.DataFrame, out_dir: Path, default_clients: int) -> None:
    ...
    emit_byzantine(df, out_dir / "byzantine_robustness.tex")
